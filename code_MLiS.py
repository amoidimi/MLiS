...
def initial_plots(pca_mod):
    ...
    plt.ylim([0, max(
        pca_comp_values['PCA0'] + pca_comp_values['PCA1'] + pca_comp_values['PCA2'] + pca_comp_values['PCA3'] +
...
def cluster_plot(Comps, Comps_test, indx, algo):
    ...
    if algo == 'kmeans':
        ...
        ax2.scatter(Comps_test[:, 0], Comps_test[:, 1], c=brcancer['Diagnosis'][indx[1]], cmap="jet", edgecolor="None",
                    alpha=0.35)
        ax2.set_title('Actual clusters')
        plt.show()
...
def cross_valid(n_folds, n_clusters, data, data_transformed, algo):
    performance_scores = {}
    nk = len(n_clusters)
    random_order = np.arange(0, brcancer_sc.shape[0], 1)
    random.seed(1243)
    random.shuffle(random_order)
    lims = np.arange(0, random_order.shape[0] - 1, 57)
    lims = np.append(lims, random_order.shape[0])
    print(lims)
    u = 1
